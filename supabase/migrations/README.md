...
58. **058_scene_party_core.sql** — Scene Party (themed group roleplay) core. Content tables `scene_packs`/`scene_characters`/`scene_act_templates` (client read-only, judge_packs pattern — new packs ship via SQL, zero app release; `is_premium` + season windows built in). State tables `scene_sessions` (1:1 with a group chat room; status casting/live/ended; `version` + `director_busy_until` = the Director-turn CAS/lease) and `scene_cast` (character claims; `user_id NULL` = AI castmate). Extends `character_chat_messages` with `message_type` ('user'/'director'/'castmate'/'game_card'/'system') + `payload jsonb`, relaxes `sender_id` to nullable with `chk_sender_by_type` (bot rows have no sender), and tightens the insert RLS so clients can ONLY create 'user' rows — bot rows come solely from the scene-director Edge Function (service role, Phase B). RPCs: `create_scene_session` (room + session + all-AI cast; premium/season gate), `claim_scene_character` (humans never displace humans), `get_scene_cast` (secret_goal only on own row). `scene_sessions` added to realtime. Run BEFORE 059.
59. **059_scene_party_seed_packs.sql** — Scene Party launch content (pure INSERTs): 4 packs × 6 characters × 3 acts — Midsummer Carnival (seasonal window), Haunted Mansion, Campus Drama, Royal Court. All free (`is_premium=false`); all content original/public-domain (no movie/show IP). New packs later = same-style INSERT script in the SQL editor.
60. **060_scene_director_engine.sql** — Scene Party Director engine (Phase B). (A) `trg_scene_msg_counters` on `character_chat_messages`: user messages increment the session's `user_msgs_in_act`/`msgs_since_director`/`msgs_since_castmate`; director/castmate inserts reset their counters — the turn policy reads these. (B) `claim_director_turn(session_id, expected_version)`: single-statement CAS + 25s lease so N clients ticking concurrently yield exactly ONE director action; EXECUTE granted to service_role only. (C) Recreates `get_scene_cast` — 058 shipped with `p.id = c.user_id` but `profiles` is keyed by `user_id`, so the cast RPC failed at runtime (empty casting grid); fixed join, same signature. (D) `trg_notify_scene_started`: in-app `scene_started` notification to human cast (except creator) when casting→live; push rides the new `scene_sessions` webhook branch in `send_battle_notification` (push-only there — no duplicate in-app rows). **After running: deploy `scene-director`, redeploy `gemini-proxy` + `send_battle_notification` (all `--use-api`), and add a Database Webhook on `public.scene_sessions` (UPDATE) → send_battle_notification.**
